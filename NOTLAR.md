...
RAG (Retrieval-Augmented Generation) LLM'lerin bilgi sınırlamalarını aşmak için geliştirilen mimaridir. LLM'ler eğitim verisindeki bilgiyyi bilir ama eğitim sonrası eklenen verilere erişemiyor. RAG bu sorunu çözüyor.LLM'e cevap üretmeden önce dış kaynaktan ilgili bilgiyi çekip context olarak veriyor.
...
Embedding: Rag'ın retireval aşamasının çalışması için metinlerin veya sorguların karşılaştırılabilir bi formata dönüşmesi gerekiyor. Bu dönüşüm embedding ile yapılır.
...
NEDEN VEKTÖR VERİTABANI KULLANIYORUZ, SQL'DE FARKI NE?
...
DAHA FAZLA ZAMANIM OLSAYDI NE EKLERDİM?
Cosine Similarity Skorlarının İyileştirilmesi(bunun için bi süre ugraştım)
...
Şu an kullanıcının sorgusunu direkt embedding'e çeviriyorum. Ama sorgu metni ile aday summary'si çok farklı formatlarda  sorgu kısa ve soru tarzında, summary ise yapısal bir özet. HyDE yaklaşımında sorgu önce LLM'e verilir, LLM hipotetik bir ideal aday profili üretir, sonra bu profil embedding'e çevrilir. Böylece doküman-doküman karşılaştırması yapılmış olur. Araştırmada gördüğüm verilere göre bu yöntem recall'da %13.8, nDCG'de %17.5 iyileşme sağlamış.
...
CLAUDE DEEP RESEARCH KULLANIDIGIM İÇİN GÖZ ATTIGI BAZI KAYNAKLAR:
